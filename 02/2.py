...
# Réessaie ceux qui n'ont pas fonctionné avec 1 chiffre en moins
Invalid = invalid_levels.copy()
for invalid_level in Invalid :
    for i in range(len(invalid_level)):
        liste = invalid_level.copy()
        liste.pop(i)
        if is_level_safe(liste):
            total += 1
